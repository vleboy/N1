...
async function promiseData(inparam) {
    ...
    // 获取区域玩家总下注次数
    promiseArr.push(queryGetSql('round.mapHandleAmount', ['betCount', 'betAmount', 'retAmount', 'refundAmount', 'winloseAmount'], inparam))
    let data = await Promise.all(promiseArr)
    return { playerCount: data[0][0], betCount: data[1][0], betAmount: data[1][1], retAmount: data[1][2], refundAmount: data[1][3], winloseAmount: data[1][4] }
}
...
// sql查询
async function queryGetSql(sqlName, keyArr, inparam) {
    ...
    for (let key of keyArr) {
        ...
        if (res.length > 0) {
            for (let item of res) {
                ...
                if (index != -1) {
                    key == 'betAmount' ? data[index].value += Math.abs(item[key]) : data[index].value += item[key]
                }
            }
        }
        // 分5组数据
        let splitList = key == 'winloseAmount' ? getSplitListForWinlose(data, 5) : getSplitList(key, data, 5)
        resData.push([data, splitList])
    }
    return resData
}
