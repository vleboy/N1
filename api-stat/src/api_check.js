 ...
 * 强制离线所有玩家
 */
router.post('/stat/checkPlayerGameState', async (ctx, next) => {
    // 查询出所有在线的玩家
 ...
 * 检查所有玩家余额和流水是否一致
 */
router.post('/stat/checkAllPlayerBalance', async (ctx, next) => {
    // 所有最近一个月登录过游戏的玩家数量
    const playerModel = new PlayerModel()
    const playerRes = await playerModel.scan({
        ProjectionExpression: 'userName,balance',
        FilterExpression: 'joinTime > :joinTime',
        ExpressionAttributeValues: { ':joinTime': Date.now() - 86400000 * 30 }
    })
    console.log(`所有最近一个月登录过游戏的玩家数量：${playerRes.Items.length}`)
    // 查询所有玩家余额，每100个玩家一组
    let i = 0
    for (let playerArr of _.chunk(playerRes.Items, 100)) {
        let promiseArr = []
        for (let player of playerArr) {
            promiseArr.push(playerModel.getNewBalance(player))
        }
        await Promise.all(promiseArr)
        console.log(i++)
    }
    console.log(`已检查${playerRes.Items.length}名玩家`)
    ctx.body = { code: 0, msg: 'Y' }
})
 ...
 * 检查单个玩家余额和流水是否一致
 */
router.post('/stat/checkSignlePlayerBalance', async (ctx, next) => {
    // 入参转换
    const inparam = ctx.request.body
    let playerRes = await new PlayerModel().getItem({
        ProjectionExpression: 'userName,balance',
        ConsistentRead: true,
        Key: { userName: inparam.userName }
    })
    //查询所有的流水并统计
    let res = await new PlayerModel().getNewBalance(playerRes.Item)
    console.log(`已检查玩家${inparam.userName}，余额${res}`)
 ...
 * 检查所有子孙关系
 */
router.post('/stat/checkSubuser', async (ctx, next) => {
    // 所有测试号
