...
class PlayerModel extends BaseModel {
    ...
    async getNewBalance(inparam) {
        ...
        let billRes = await new PlayerBillDetailModel().queryByTime({ userName, createdAt: cacheItem.lastTime })
        if (billRes && billRes.Items && billRes.Items.length > 0) {
            ...
            if (balance.toFixed(2) != playerBalance.toFixed(2)) {
                console.error(`玩家${userName}余额和流水汇总余额不一致:玩家表余额${playerBalance},流水汇总余额${balance}`)
            ...
            if (lastBalance.toFixed(2) != playerBalance.toFixed(2)) {
                console.error(`玩家${userName}余额和流水最后余额不一致:玩家表余额${playerBalance},流水最后余额${lastBalance}`)
                isAllowUpdateCache = false
            }
            //更新缓存
