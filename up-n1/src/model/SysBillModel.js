...
class SysBillModel extends BaseModel {
    ...
    async calcManagerStat(inparam) {
        ...
        for (let userId of inparam.userIds) {
            let p = new Promise(async function (resolve, reject) {
                ...
                const merchantsRet = await self.query(query)
                console.log(`线路商的下级商户数量：${merchantsRet.Items.length}`)
                let userIds = []
                for (let merchant of merchantsRet.Items) {
                    userIds.push(merchant.userId)
                }
                // 查询对应所有玩家统计数据并返回
    ...
    async calcAdminStat(inparam) {
        ...
        const merchantsRet = await this.query(query)
        console.log(`平台所有商户数量：${merchantsRet.Items.length}`)
        let userIds = []
        for (let merchant of merchantsRet.Items) {
            userIds.push(merchant.userId)
        }
