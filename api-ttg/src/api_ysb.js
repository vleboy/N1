    ...
    if (UN.length == 8) {
        ...
        switch (action) {
            case 'ACCOUNTBALANCE':
                ...
                ctx.body = getYSBResponse(action, { UN: `NAPL_${UN}`, CC, BAL: n2res.data.balance, S })
                break;
            // 设置为冻结
            case 'BET':
                bill.type = 3
            ...
            case 'BETCONFIRM':
                ...
                if (syncRes && !syncRes.err) {
                    ctx.body = getYSBResponse(action, { TRX, UN: `NAPL_${UN}`, CC, BAL: syncRes.balance, S })
                }
                break;
            case 'PAYOUT':
                bill.type = 4
                ...
                syncRes = await syncBill(bill)
                if (syncRes && !syncRes.err) {
                    ctx.body = getYSBResponse(action, { TRX, UN: `NAPL_${UN}`, CC, BAL: syncRes.balance, S, ED })
                }
                break;
            default:
                return
